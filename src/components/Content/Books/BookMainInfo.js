...
function BookMainInfo(props) {
    ...
    let descriptionBlk = <div 
                         className="book-container__description">
                            <h4>Description</h4>
                            <p>
                                {props.dscrpt ? props.dscrpt : "No Description Available..."}
                            </p>
                        </div>
    ...
    return (
        <div 
        className="book-container__gen-info" 
        data-ref={props.type}>
            <div>
                <strong>#{props.rank}</strong>
            </div>
            <div 
            className="book-container__cover" 
            data-ref={props.type}>
                <img 
                onClick={handleBkClick} 
                src={bookCover} 
                alt={props.title}/>
                {props.type === 'genre' ? descriptionBlk : <></>}
            </div>
            <div 
            className="book-container__title-author">
                <button 
                ...
                <div 
                className="book-container__author-info">
                    <p>by</p>
                    <div 
                    className="book-container__author-btns" 
                    data-ref={props.type}>
                        {authorArr}
                    </div>
                </div>
